...
const ProfileHeader: FC<ProfileHeaderProps> = ({ user, layoutTab = 1 }) => {
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <p className="text-[#B0B0B0]">
                  {stats?.total_xp || 0} /{" "}
                  {stats?.level_info?.nextLevelXP || 100} XP
                </p>
                <p className="text-[#A0FF75] font-semibold">
                  {stats?.level_info?.progressPercent || 0}%
                </p>
              </div>
              <div className="h-2 bg-[#181B22] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${stats?.level_info?.progressPercent || 0}%`,
                  }}
                ></div>
              </div>
              <p className="text-xs text-[#B0B0B0]">
                {(stats?.level_info?.nextLevelXP || 100) -
                  (stats?.total_xp || 0)}{" "}
                XP to reach Level {(stats?.current_level || 1) + 1}
              </p>
              <button
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 pt-1">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-[#A06AFF] font-semibold">
                      72%
                    </span>
                  </div>
  ...
  if (layoutTab === 2) {
    return (
      <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-5 w-full">
        ...
        <RightPanelLayout2
          stats={stats}
          onEditClick={() => setShowStatsModal(true)}
        />
        <AdminStatsModal
          open={showStatsModal}
          onOpenChange={setShowStatsModal}
                  ...
                  <div className="relative w-full h-40 px-4">
                    ...
                    >
                      ...
                      <g clipPath="url(#chart-progress-clip-main)">
                        <path d={areaD} fill="url(#filled-gradient-main)" />
  ...
  if (layoutTab === 4) {
    const progressPercent = stats?.level_info?.progressPercent || 0;
    const xpLeft =
      (stats?.level_info?.nextLevelXP || 100) - (stats?.total_xp || 0);
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#B0B0B0]">
                {progressPercent}% · {xpLeft} XP left
              </span>
            </div>
          </div>
    ...
    <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-5 w-full">
      <div className="lg:col-span-3 space-y-4">
        ...
        <div className="rounded-3xl border border-[#181B22] bg-[#0C101480] p-4 sm:p-6">
          <h3 className="text-xs sm:text-sm font-bold text-white uppercase tracking-wide mb-4">
            Achievements
          </h3>
