import { ShoppingCart, MessageCircle, Star, Users, Edit } from "lucide-react";
...
const TRADER_AVATAR = "https://cdn.builder.io/api/v1/image/assets%2F684cb122a7e14784926e57d7235fa702%2F68315e5814ee44f2b3af7585af3ac179?format=webp&width=800";
...
const TraderDetailLanding: FC = () => {
  ...
  const locationState = (location.state as { trader?: Trader } | null) ?? null;
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(FAVORITE_STORAGE_KEY, JSON.stringify(Array.from(favoriteTraderIds)));
  }, [favoriteTraderIds]);

  const trader = useMemo<Trader>(() => {
    const fallback: Trader = {
      id: "default-trader",
      name: "Sarah Lee",
      avatar: TRADER_AVATAR,
    ...
    const provided = locationState?.trader;
  ...
  const handleNavigateToCategory = useCallback(() => {
    navigate("/marketplace/traders", { state: { scrollToTop: true } });
  }, [navigate]);

  const reviews = useMemo(() => {
    const sourceReviews = Array.isArray(trader.reviews) ? trader.reviews : [];
            ...
            <div className="flex flex-col overflow-hidden rounded-2xl border border-[#181B22] bg-[#0C1014]/50 backdrop-blur-[50px]">
              ...
              <div className="flex flex-col gap-2 p-4">
                ...
                <div className="flex flex-wrap items-center justify-center gap-1">
                  <div className="rounded bg-[#3E321D] px-1 py-0.5">
                    <span className="text-xs font-bold uppercase text-[#FFA800]">{trader.badge}</span>
                  </div>
                ...
                <p className="text-xs font-bold uppercase text-[#B0B0B0] text-center mt-2">{trader.certification}</p>
              ...
              <div className="border-t border-[#181B22] px-4 py-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">Trades (30d):</span>
                  <span className="text-xs font-bold uppercase text-white">{trader.trades30Days}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">Experience:</span>
                  <span className="text-xs font-bold uppercase text-white">{trader.experience}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">ROI (Month):</span>
                  <span className="text-xs font-bold uppercase text-[#2EBD85]">{trader.roiMonth}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">ROI (Quarter):</span>
                  <span className="text-xs font-bold uppercase text-[#2EBD85]">{trader.roiQuarter}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">Avg Profit:</span>
                  <span className="text-xs font-bold uppercase text-[#2EBD85]">{trader.avgProfitability}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-[#B0B0B0]">Accuracy:</span>
                  <span className="text-xs font-bold uppercase text-[#2EBD85]">{trader.accuracy}</span>
                </div>
              </div>

              <div className="border-t border-[#181B22] p-4">
                <p className="text-2xl font-bold text-white">{trader.price}</p>
              </div>
          ...
          <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex gap-6">
              <div className="flex flex-1 flex-col items-center gap-4 rounded-2xl border border-[#181B22] bg-[#0C1014]/50 p-4 backdrop-blur-[50px]">
                ...
                <p className="text-center text-xs font-bold text-white">{trader.successRate} profitable transactions</p>
              </div>
              ...
              <div className="flex flex-1 flex-col items-center justify-between gap-4 rounded-2xl border border-[#181B22] bg-[#0C1014]/50 p-4 backdrop-blur-[50px]">
                ...
                <p className="text-center text-[31px] font-bold text-[#2EBD85]">{trader.averageReturn}</p>
            ...
            <div className="flex flex-col rounded-3xl border border-[#181B22] bg-[#0C1014]/50 backdrop-blur-[50px]">
              <div className="flex items-center gap-2 p-4">
                ...
                <div className="flex flex-1 flex-col gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-[15px] font-bold text-white">{trader.name}</span>
              ...
              {trader.communityLink && (
                <div className="px-4 pb-4">
                  <p className="text-[15px] font-medium text-[#B0B0B0]">
                    Join our 10k+ community:{" "}
                    <a href={trader.communityLink} target="_blank" rel="noreferrer" className="text-[#A06AFF] underline">
                      example.com
                    </a>
                  </p>
                </div>
              ...
              {trader.bio && (
                <div className="px-4">
                  <p className="text-[15px] font-medium text-[#B0B0B0]">{trader.bio}</p>
                </div>
              )}

              {trader.socials && trader.socials.length > 0 && (
                <div className="flex items-center gap-3 px-4 pt-4">
                  <span className="text-[15px] font-medium text-[#B0B0B0]">Also on:</span>
                  <div className="flex items-center gap-3">
                    {trader.socials.map((social, idx) => (
                ...
                <div className="border-b border-[#181B22] p-4">
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-4">
                      <p className="text-[31px] font-bold text-[#A06AFF]">{trader.averageRating}</p>
                      <div className="flex flex-col gap-1">
                        <div className="flex gap-0.5">{renderStars(trader.averageRating ?? 0)}</div>
                        <p className="text-xs font-bold text-[#B0B0B0]">Based on {trader.totalReviews} reviews</p>
                      </div>
                    </div>
                  </div>
                </div>
